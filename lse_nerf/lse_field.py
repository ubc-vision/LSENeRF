...
from nerfstudio.field_components.encodings import SHEncoding, HashEncoding, Encoding
...
class Ed_HashEncoding(HashEncoding):
    def __init__(self, num_levels: int = 16, 
                       ...
                       interpolation: Literal['Nearest', 'Linear', 'Smoothstep'] = None) -> None:
        Encoding.__init__(self, in_dim=3)
...
class LSEField(Field):
    ...
    def __init__(
        ...
        embd_config: LSEEmbeddingConfig = None,
        ...
        if self.appearance_embedding_dim > 0:
            ...
            self.embedding_appearance = embd_config.setup(num_imgs=self.num_images, 
                                                          num_dims=self.appearance_embedding_dim  # NOTE: num_dims is overide by EdEmbeddingConfig
                                                          )
            self.appearance_embedding_dim = self.embedding_appearance.get_emb_dim() if self.embedding_appearance is not None else 0
        else:
        ...
        self.mlp_base_grid = Ed_HashEncoding(
    ...
    def get_outputs(
        ...
        if self.embedding_appearance is not None:
            if self.training:
                embedded_appearance = self.embedding_appearance(ray_samples)
            else:
                embedded_appearance = self.embedding_appearance.get_test_emb(ray_samples)
